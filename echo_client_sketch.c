...
bool buffers_equal(ptk_buf *buf1, ptk_buf *buf2) {
    ...
    buf_size_t len1 = ptk_buf_get_len(buf1);
    buf_size_t len2 = ptk_buf_get_len(buf2);
...
int main(int argc, char *argv[]) {
    ...
    // Summary
    info("Echo test complete: %d/%d messages echoed successfully", successful_echoes, message_count);
...
cleanup:
    ...
    if(send_buffer) { ptk_free(send_buffer); }

    if(recv_buffer) { ptk_free(recv_buffer); }
