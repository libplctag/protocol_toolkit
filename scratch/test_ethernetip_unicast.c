...
static void udp_response_handler(const ev_event *event) {
    ...
    switch(event->type) {
        case EV_EVENT_READ: {
            ...
            info("Received UDP response from %s:%d, %zu bytes", 
...
static void send_list_identity_to_host(ev_sock *udp_sock, const char *host) {
    ...
    ev_err send_result = ev_udp_send(udp_sock, &request_buf, host, 2222);
    if(send_result != EV_OK) {
        error("Failed to send request to %s: %s (error code: %d)", host, ev_err_string(send_result), send_result);
    } else {
...
int main() {
    ...
    ev_udp_opts udp_opts = {
        .bind_host = NULL,  // Don't bind at all
