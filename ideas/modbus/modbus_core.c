...
ptk_err modbus_recv_frame(modbus_connection *conn, ptk_buf_t *pdu_buf) {
    ...
    if(err != PTK_OK) {
        ptk_buf_dispose(frame_buf);
        return err;
    }

    // Validate header
    if(protocol_id != 0 || unit_id != conn->unit_id) {
        ptk_buf_dispose(frame_buf);
    ...
    if(pdu_len > MODBUS_MAX_PDU_SIZE) {
        ptk_buf_dispose(frame_buf);
        return PTK_ERR_PROTOCOL_ERROR;
    }
...
modbus_connection *server_accept_connection(modbus_connection *server_connection) {
    if(!server_connection || !server_connection->is_server) { return NULL; }
