...
ptk_err client_send_read_holding_register_req(modbus_connection *conn, uint16_t register_addr) {
    ...
    ptk_err err = ptk_buf_produce(pdu_buf, ">bww", MODBUS_FC_READ_HOLDING_REGISTERS, register_addr, (uint16_t)1);
...
ptk_err client_send_read_holding_registers_req(modbus_connection *conn, uint16_t base_register, uint16_t num_registers) {
    ...
    // Produce PDU: function_code, starting_address, quantity
    ptk_err err = ptk_buf_produce(pdu_buf, ">bww", MODBUS_FC_READ_HOLDING_REGISTERS, base_register, num_registers);
...
ptk_err client_send_write_holding_register_req(modbus_connection *conn, uint16_t register_addr, uint16_t register_value) {
    ...
    ptk_err err = ptk_buf_produce(pdu_buf, ">bww", MODBUS_FC_WRITE_SINGLE_REGISTER, register_addr, register_value);
...
ptk_err client_send_write_holding_registers_req(modbus_connection *conn, uint16_t base_register,
    ...
    // Create buffer with capacity for header + PDU
    size_t pdu_size = 6 + (num_registers * 2);  // function_code + starting_address + quantity + byte_count + register_values
    ...
    ptk_err err = ptk_buf_produce(pdu_buf, ">bwwb", MODBUS_FC_WRITE_MULTIPLE_REGISTERS, base_register, (uint16_t)num_registers,
...
ptk_err client_recv_read_read_holding_register_resp(modbus_connection *conn, uint16_t *register_value) {
    ...
    // Validate response
    if(function_code != MODBUS_FC_READ_HOLDING_REGISTERS || byte_count != 2) {
        ptk_buf_dispose(pdu_buf);
...
ptk_err client_recv_read_read_holding_registers_resp(modbus_connection *conn, modbus_register_array_t **register_values) {
    ...
    // Validate response
    if(function_code != MODBUS_FC_READ_HOLDING_REGISTERS || byte_count % 2 != 0) {
        ptk_buf_dispose(pdu_buf);
        return PTK_ERR_PROTOCOL_ERROR;
    }
...
ptk_err client_recv_write_holding_registers_resp(modbus_connection *conn) {
    ...
    // Validate response
    if(function_code != MODBUS_FC_WRITE_MULTIPLE_REGISTERS) {
        ptk_buf_dispose(pdu_buf);
        return PTK_ERR_PROTOCOL_ERROR;
    }
...
ptk_err server_recv_read_holding_register_req(modbus_connection *conn, uint16_t *register_addr) {
    ...
    // Validate request
    if(function_code != MODBUS_FC_READ_HOLDING_REGISTERS || quantity != 1) {
        ptk_buf_dispose(pdu_buf);
        return PTK_ERR_INVALID_PARAM;
    }
...
ptk_err server_recv_write_holding_register_req(modbus_connection *conn, uint16_t *register_addr, uint16_t *register_value) {
    if(!conn || !register_addr || !register_value || conn->is_server) { return PTK_ERR_INVALID_PARAM; }
...
ptk_err server_recv_write_holding_registers_req(modbus_connection *conn, uint16_t *base_register,
                                                modbus_register_array_t **register_values) {
    if(!conn || !base_register || !register_values || conn->is_server) { return PTK_ERR_INVALID_PARAM; }
    ...
    // Validate request
    if(function_code != MODBUS_FC_WRITE_MULTIPLE_REGISTERS || byte_count != quantity * 2) {
        ptk_buf_dispose(pdu_buf);
        return PTK_ERR_INVALID_PARAM;
    }
...
ptk_err server_send_read_holding_register_resp(modbus_connection *conn, uint16_t register_value) {
    ...
    ptk_err err = ptk_buf_produce(pdu_buf, ">bbw", MODBUS_FC_READ_HOLDING_REGISTERS,
...
ptk_err server_send_read_holding_registers_resp(modbus_connection *conn, const modbus_register_array_t *register_values) {
    ...
    ptk_err err = ptk_buf_produce(pdu_buf, ">bb", MODBUS_FC_READ_HOLDING_REGISTERS, (uint8_t)(num_registers * 2));
...
ptk_err server_send_write_holding_register_resp(modbus_connection *conn) {
    ...
    ptk_err err = ptk_buf_produce(pdu_buf, ">bww", MODBUS_FC_WRITE_SINGLE_REGISTER,
...
ptk_err server_send_write_holding_registers_resp(modbus_connection *conn) {
    ...
    // Note: In a real implementation, you'd store the original request values
    // For now, this is a placeholder that assumes success
    ptk_err err = ptk_buf_produce(pdu_buf, ">bww", MODBUS_FC_WRITE_MULTIPLE_REGISTERS,
