...
ptk_err client_send_read_discrete_input_req(modbus_connection *conn, uint16_t input_addr) {
    ...
    ptk_err err = ptk_buf_produce(pdu_buf, ">bww", MODBUS_FC_READ_DISCRETE_INPUTS, input_addr, (uint16_t)1);
...
ptk_err client_send_read_discrete_inputs_req(modbus_connection *conn, uint16_t base_input, uint16_t num_inputs) {
    if(!conn || conn->is_server || num_inputs == 0 || num_inputs > 2000) { return PTK_ERR_INVALID_PARAM; }
    ...
    // Produce PDU: function_code, starting_address, quantity
    ptk_err err = ptk_buf_produce(pdu_buf, ">bww", MODBUS_FC_READ_DISCRETE_INPUTS, base_input, num_inputs);
...
ptk_err client_recv_read_discrete_input_resp(modbus_connection *conn, bool *input_value) {
    if(!conn || !input_value || conn->is_server) { return PTK_ERR_INVALID_PARAM; }
    ...
    // Validate response
    if(function_code != MODBUS_FC_READ_DISCRETE_INPUTS || byte_count != 1) {
        ptk_buf_dispose(pdu_buf);
        return PTK_ERR_PROTOCOL_ERROR;
    }

    *input_value = (input_status & 0x01) != 0;
    ptk_buf_dispose(pdu_buf);
    return PTK_OK;
}

ptk_err client_recv_read_discrete_inputs_resp(modbus_connection *conn, modbus_bool_array_t **input_values) {
    if(!conn || !input_values || conn->is_server) { return PTK_ERR_INVALID_PARAM; }
...
ptk_err server_recv_read_discrete_input_req(modbus_connection *conn, uint16_t *input_addr) {
    if(!conn || !input_addr || conn->is_server) { return PTK_ERR_INVALID_PARAM; }
    ...
    // Validate request
    if(function_code != MODBUS_FC_READ_DISCRETE_INPUTS || quantity != 1) {
        ptk_buf_dispose(pdu_buf);
        return PTK_ERR_INVALID_PARAM;
    }

    *input_addr = starting_address;
    ptk_buf_dispose(pdu_buf);
    return PTK_OK;
}

ptk_err server_recv_read_discrete_inputs_req(modbus_connection *conn, uint16_t *base_input, uint16_t *num_inputs) {
    if(!conn || !base_input || !num_inputs || conn->is_server) { return PTK_ERR_INVALID_PARAM; }
...
ptk_err server_send_read_discrete_input_resp(modbus_connection *conn, bool input_value) {
    ...
    // Create PDU buffer
    ptk_buf_t *pdu_buf = ptk_buf_create(conn->allocator, MODBUS_HEADER_SIZE + 3);
    if(!pdu_buf) { return PTK_ERR_NO_RESOURCES; }

    // Produce response PDU: function_code, byte_count, input_status
    ptk_err err = ptk_buf_produce(pdu_buf, ">bbb", MODBUS_FC_READ_DISCRETE_INPUTS,
...
ptk_err server_send_read_discrete_inputs_resp(modbus_connection *conn, const modbus_bool_array_t *input_values) {
    ...
    if(num_inputs == 0 || num_inputs > 2000) { return PTK_ERR_INVALID_PARAM; }
    ...
    ptk_err err = ptk_buf_produce(pdu_buf, ">bb", MODBUS_FC_READ_DISCRETE_INPUTS, (uint8_t)byte_count);
