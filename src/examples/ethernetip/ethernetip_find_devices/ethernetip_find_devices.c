 ...
 */
static ptk_err parse_list_identity_response(ptk_buf *buffer, const char *sender_ip, int sender_port) {
    ptk_err err;
        ...
        for (uint16_t i = 0; i < item_count; i++) {
            ...
            if (type_id == CPF_TYPE_CIP_IDENTITY && item_length >= 34) {
                ...
                printf("    Socket Address: %u.%u.%u.%u:%u\n", 
                       addr_bytes[0], addr_bytes[1], addr_bytes[2], addr_bytes[3], sin_port);
            } else {
...
static void discovery_thread(void *arg) {
    ...
    if (err != PTK_OK) {
        ...
        for (size_t i = 0; i < num_networks; i++) {
            printf("  %zu: IP=%s, Netmask=%s, Broadcast=%s\n", 
                   i + 1, networks[i].network_ip, networks[i].netmask, networks[i].broadcast);
        }
    }
    ...
    if (err != PTK_OK) {
        ...
        if (networks) {
            ptk_socket_network_info_dispose(networks, num_networks);
        }
        return;
    }
            ...
            if (err == PTK_OK) {
                ...
                if (networks && num_networks > 0) {
                    for (size_t i = 0; i < num_networks; i++) {
                        ...
                        err = ptk_udp_socket_send(g_udp_socket, &request_buf, 
                                                networks[i].broadcast, EIP_PORT, true);
                        if (err == PTK_OK) {
...
int main(int argc, char *argv[]) {
    ...
    if (err != PTK_OK) {
        printf("Error joining discovery thread: %s\n", ptk_err_to_string(err));
    }
    
    // Cleanup
    if (g_udp_socket) {
        ptk_socket_close(g_udp_socket);
    }
    
    if (g_discovery_thread) {
        ptk_thread_destroy(g_discovery_thread);
    }
