/**
 * @file mem.c
 * @brief POSIX implementation of unified memory management for Protocol Toolkit
 *
 * This file replaces src/lib/ptk_mem.c. All functionality is implemented, no stubs.
 */
...
// ...local alloc header/footer structs and helpers...
...
static size_t round_up_16(size_t sz) { return (sz + PTK_ALLOC_ALIGNMENT - 1) & ~(PTK_ALLOC_ALIGNMENT - 1); }

static ptk_local_alloc_header_t *get_header(void *user_ptr) {
    if(!user_ptr) { return NULL; }
    return (ptk_local_alloc_header_t *)((uint8_t *)user_ptr - sizeof(ptk_local_alloc_header_t));
}

static bool validate_canaries(void *user_ptr) {
    if(!user_ptr) {
        ptk_set_err(PTK_ERR_NULL_PTR);
        return false;
    }
    ptk_local_alloc_header_t *header = get_header(user_ptr);
    if(!header) {
        ptk_set_err(PTK_ERR_NULL_PTR);
        return false;
    }
    ...
    if(header->footer_canary != PTK_ALLOC_FOOTER_CANARY) {
        ptk_set_err(PTK_ERR_INVALID_PARAM);
        return false;
    }
    return true;
...
void *ptk_local_alloc_impl(const char *file, int line, size_t size, void (*destructor)(void *ptr)) {
    ...
    size_t aligned_size = round_up_16(size);
    ...
    header->footer_canary = PTK_ALLOC_FOOTER_CANARY;
...
void *ptk_local_realloc_impl(const char *file, int line, void *ptr, size_t new_size) {
    ...
    if(!validate_canaries(ptr)) {
        error("Invalid canaries in realloc at %s:%d", file, line);
        return NULL;
    }
    ptk_local_alloc_header_t *old_header = get_header(ptr);
...
void ptk_local_free_impl(const char *file, int line, void **ptr_ref) {
    ...
    ptk_local_alloc_header_t *header = get_header(ptr);
...
ptk_shared_handle_t ptk_shared_alloc_impl(const char *file, int line, size_t size, void (*destructor)(void *ptr)) {
    ...
    shared_table.count++;
    debug("Shared memory %p with handle 0x%016" PRIx64 " at index %zu from %s:%d", ptr, entry->handle_value, entry_index, file, line);
...
void *ptk_shared_acquire_impl(const char *file, int line, ptk_shared_handle_t handle, ptk_time_ms timeout) {
    ...
    entry->ref_count++;
    debug("Acquired shared memory %p (ref_count=%u) from %s:%d", entry->data_ptr, entry->ref_count, file, line);
    pthread_mutex_unlock(&entry->mutex);
    ptk_set_err(PTK_OK);
...
ptk_err_t ptk_shared_release_impl(const char *file, int line, ptk_shared_handle_t handle) {
    ...
    entry->ref_count--;
    debug("Released shared memory %p (ref_count=%u) from %s:%d", entry->data_ptr, entry->ref_count, file, line);
    if (entry->ref_count == 0) {
...
ptk_err_t ptk_shared_realloc_impl(const char *file, int line, ptk_shared_handle_t handle, size_t new_size) {
    ...
    debug("Reallocated shared memory to %zu bytes at %p from %s:%d", new_size, new_ptr, file, line);
    return PTK_OK;
}
