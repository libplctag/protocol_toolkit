...
ptk_err ptk_buf_set_end(ptk_buf *buf, size_t end) {
    ...
    if (end < buf->start || end > buf->capacity) {
        return PTK_ERR_OUT_OF_BOUNDS;
    }
...
ptk_err ptk_buf_move_to(ptk_buf *buf, size_t new_start) {
    ...
    if (new_start + data_len > buf->capacity) {
        return PTK_ERR_OUT_OF_BOUNDS;
    }
    
    // Move data if there's any and positions are different
    if (data_len > 0 && new_start != buf->start) {
        memmove(buf->data + new_start, buf->data + buf->start, data_len);
    }
...
ptk_err ptk_buf_produce_u64(ptk_buf *buf, uint64_t value, ptk_buf_endianness endianness) {
    ...
    switch (endianness) {
        ...
        case PTK_BUF_LITTLE_ENDIAN_BYTE_SWAP:
            for (int i = 0; i < 8; i++) {
                buf->data[buf->end + i] = (value >> (56 - i * 8)) & 0xFF;
            }
            break;
        case PTK_BUF_LITTLE_ENDIAN:
        case PTK_BUF_BIG_ENDIAN_BYTE_SWAP:
            for (int i = 0; i < 8; i++) {
                buf->data[buf->end + i] = (value >> (i * 8)) & 0xFF;
            }
            break;
    }
...
ptk_err ptk_buf_consume_u16(ptk_buf *buf, uint16_t *value, ptk_buf_endianness endianness, bool peek) {
    ...
    switch (endianness) {
        case PTK_BUF_BIG_ENDIAN:
        case PTK_BUF_LITTLE_ENDIAN_BYTE_SWAP:
            *value = ((uint16_t)buf->data[buf->start] << 8) | 
                     (uint16_t)buf->data[buf->start + 1];
            break;
        case PTK_BUF_LITTLE_ENDIAN:
        case PTK_BUF_BIG_ENDIAN_BYTE_SWAP:
            *value = (uint16_t)buf->data[buf->start] | 
                     ((uint16_t)buf->data[buf->start + 1] << 8);
            break;
    }
...
ptk_err ptk_buf_consume_u32(ptk_buf *buf, uint32_t *value, ptk_buf_endianness endianness, bool peek) {
    ...
    switch (endianness) {
        case PTK_BUF_BIG_ENDIAN:
        case PTK_BUF_LITTLE_ENDIAN_BYTE_SWAP:
            *value = ((uint32_t)buf->data[buf->start] << 24) |
                     ((uint32_t)buf->data[buf->start + 1] << 16) |
                     ((uint32_t)buf->data[buf->start + 2] << 8) |
                     (uint32_t)buf->data[buf->start + 3];
            break;
        case PTK_BUF_LITTLE_ENDIAN:
        case PTK_BUF_BIG_ENDIAN_BYTE_SWAP:
            *value = (uint32_t)buf->data[buf->start] |
                     ((uint32_t)buf->data[buf->start + 1] << 8) |
                     ((uint32_t)buf->data[buf->start + 2] << 16) |
                     ((uint32_t)buf->data[buf->start + 3] << 24);
            break;
    }
...
ptk_err ptk_buf_consume_u64(ptk_buf *buf, uint64_t *value, ptk_buf_endianness endianness, bool peek) {
    ...
    *value = 0;
    ...
    switch (endianness) {
        ...
        case PTK_BUF_LITTLE_ENDIAN_BYTE_SWAP:
            for (int i = 0; i < 8; i++) {
                *value |= ((uint64_t)buf->data[buf->start + i]) << (56 - i * 8);
            }
            break;
        case PTK_BUF_LITTLE_ENDIAN:
        case PTK_BUF_BIG_ENDIAN_BYTE_SWAP:
            for (int i = 0; i < 8; i++) {
                *value |= ((uint64_t)buf->data[buf->start + i]) << (i * 8);
            }
            break;
    }
