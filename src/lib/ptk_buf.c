...
static size_t calculate_format_size(const char *fmt, va_list args) {
    ...
    while(*p) {
        ...
        switch(*p) {
            ...
            default: error("Unknown format character '%c'", *p); return 0;
        }
...
static ptk_err write_value_to_buffer(u8 *dest, uint64_t value, size_t size, bool little_endian) {
    if(little_endian) {
        for(size_t i = 0; i < size; i++) { dest[i] = (value >> (i * 8)) & 0xFF; }
    } else {
        for(size_t i = 0; i < size; i++) { dest[size - 1 - i] = (value >> (i * 8)) & 0xFF; }
    }
    return PTK_OK;
}
...
static uint64_t read_value_from_buffer(const u8 *src, size_t size, bool little_endian) {
    ...
    if(little_endian) {
        for(size_t i = 0; i < size; i++) { value |= ((uint64_t)src[i]) << (i * 8); }
    } else {
        for(size_t i = 0; i < size; i++) { value |= ((uint64_t)src[size - 1 - i]) << (i * 8); }
    }

    return value;
}

ptk_err ptk_buf_produce(ptk_buf *dest, const char *fmt, ...) {
    if(!dest || !fmt) {
...
ptk_err ptk_buf_consume(ptk_buf *src, bool peek, const char *fmt, ...) {
    if(!src || !fmt) {
    ...
    if(result == PTK_OK && num_backups < 64) {
        ...
        for(size_t i = 0; i < num_backups; i++) {
            if(arg_backups[i].is_pointer) { free((void *)arg_backups[i].original_value); }
        }
    } else {
