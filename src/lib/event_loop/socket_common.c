 ...
 * The buffer must have enough space to hold the incoming data; this function does not handle partial reads beyond what recv() provides.
 * @param sock TCP client socket (must be valid and of type PTK_SOCK_TCP_CLIENT).
...
ptk_err ptk_udp_socket_send_to(ptk_sock *sock, ptk_buf *data, const ptk_address_t *dest_addr, bool broadcast, ptk_duration_ms timeout_ms) {
    ...
    int fd = sock->fd;
    ssize_t bytes_sent = sendto(fd, data->data + data->start, data->end - data->start, MSG_DONTWAIT,
 ...
 * If the socket is invalid or arguments are bad, it returns PTK_ERR_INVALID_ARGUMENT.
 * If the socket was aborted while waiting, it returns PTK_ERR_ABORT (handled by threadlet status).
 * The buffer must have enough space to hold the incoming data; this function does not handle partial reads beyond what recvfrom() provides.
 * @param sock UDP socket (must be valid and of type PTK_SOCK_UDP).
 * @param data Buffer to read data into (must be valid and have enough space).
