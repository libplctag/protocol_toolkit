...
static void* debug_alloc(ptk_allocator_t *allocator, size_t size);
...
typedef enum {
    PTK_ALLOCATOR_DEFAULT,
    PTK_ALLOCATOR_DEBUG, 
    PTK_ALLOCATOR_ARENA
} ptk_allocator_type_t;
...
static void* default_alloc(ptk_allocator_t *allocator, size_t size) {
...
static void debug_mark_freed(debug_allocator_t *debug_alloc, void *ptr) {
    ...
    if (entry->freed) {
        error("Double free detected for pointer %p (originally allocated at %s:%d)", 
              ptr, entry->file ? entry->file : "unknown", entry->line);
        return;
    }
...
static void* debug_realloc(ptk_allocator_t *allocator, void *ptr, size_t new_size) {
    ...
    if (!ptr) {
        return debug_alloc(allocator, new_size);
    }
...
static void debug_reset(ptk_allocator_t *allocator) {
    ...
    while (current) {
        ...
        if (!current->freed) {
            info("Cleaning up unfreed allocation %p (%zu bytes)", current->ptr, current->size);
        }
    ...
    info("Debug allocator reset complete: freed %zu allocations totaling %zu bytes", 
...
void ptk_debug_allocator_report(const ptk_allocator_t *allocator) {
    ...
    while (current) {
        if (!current->freed) {
            ...
            warn("LEAK: %p (%zu bytes) allocated at %s:%d", 
    ...
    if (freed_but_not_cleaned > 0) {
        info("%zu freed allocations pending cleanup", freed_but_not_cleaned);
    }
...
static void* arena_alloc(ptk_allocator_t *allocator, size_t size) {
    arena_allocator_t *arena = (arena_allocator_t*)allocator;
    size_t aligned_size = PTK_ALIGN_SIZE(size, allocator->default_alignment);
    
    if (arena->offset + aligned_size > arena->size) {
        error("Arena allocator out of memory: requested %zu bytes, %zu available", 
    ...
    if (arena->stats.total_allocated > arena->stats.peak_allocated) {
        arena->stats.peak_allocated = arena->stats.total_allocated;
    }
    
    trace("Arena allocated %zu bytes at %p (%zu/%zu used)", 
          aligned_size, ptr, arena->offset, arena->size);
    return ptr;
}
...
static void* arena_realloc(ptk_allocator_t *allocator, void *ptr, size_t new_size) {
    ...
    if (!ptr) {
        return arena_alloc(allocator, new_size);
    }
...
ptk_allocator_t *allocator_arena_create(size_t pool_size, size_t default_alignment) {
    ...
    debug("Created arena allocator with %zu bytes and %zu-byte alignment", 
          pool_size, default_alignment);
    return &alloc->base;
}
