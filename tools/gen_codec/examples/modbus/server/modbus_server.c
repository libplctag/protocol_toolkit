...
ptk_err encode_modbus_tcp_header(ptk_buf_t *buf, const modbus_tcp_header_t *header) {
    ptk_err err;
...
ptk_err decode_modbus_tcp_header(ptk_buf_t *buf, modbus_tcp_header_t *header) {
    ptk_err err;
...
ptk_err process_read_coils(ptk_allocator_t *alloc, ptk_buf_t *request_buf, 
                          ptk_buf_t *response_buf, const modbus_tcp_header_t *req_header) {
    ptk_err err;
    ...
    for (int byte_idx = 0; byte_idx < byte_count; byte_idx++) {
        ...
        for (int bit_idx = 0; bit_idx < 8; bit_idx++) {
            ...
            if (coil_idx < start_addr + quantity && g_server_state.coils[coil_idx]) {
                coil_byte |= (1 << bit_idx);
            }
        }
...
ptk_err process_read_holding_registers(ptk_allocator_t *alloc, ptk_buf_t *request_buf,
                                     ptk_buf_t *response_buf, const modbus_tcp_header_t *req_header) {
    ptk_err err;
    uint8_t function_code;
    uint16_t start_addr, quantity;
...
ptk_err process_modbus_request(ptk_allocator_t *alloc, ptk_buf_t *request_buf, ptk_buf_t *response_buf) {
    ptk_err err;
    ...
    ptk_log_info("Modbus request: transaction_id=%u, length=%u, unit_id=%u", 
         header.transaction_id, header.length, header.unit_id);
    ...
    switch (function_code) {
        case READ_COILS:
            return process_read_coils(alloc, request_buf, response_buf, &header);
            
        case READ_HOLDING_REGISTERS:
            return process_read_holding_registers(alloc, request_buf, response_buf, &header);
...
ptk_err handle_client(ptk_allocator_t *alloc, ptk_sock *client) {
    ptk_err err;
    ptk_buf_t *request_buf = NULL;
    ptk_buf_t *response_buf = NULL;
